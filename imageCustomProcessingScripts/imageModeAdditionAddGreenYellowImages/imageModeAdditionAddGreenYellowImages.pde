...
void setup() {
  size(1024, 768);
  origA = loadImage("yellowLeaves.jpg");
  origB = loadImage("greenLion.jpg");
}
...
//loop through all pixels in newA and assign RGB color to each pixel
 ...
 newA.pixels[loc] = color(Cr[loc], Cg[loc], Cb[loc]);
    }//end loop 1
  }//end loop 2
 ...
 newB.pixels[loc] = color(Cr[loc], Cg[loc], Cb[loc]);
    }//end loop 1
  }//end loop 2
...
popMatrix();
  ...
  //save the images
  //save("newImageA_addRGB.png");
  save("newImageB_addRGB.png");
